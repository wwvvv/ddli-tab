...
- 该文件不是已经完成的同步服务。登录 UI、自动同步调度、离线队列、冲突选择界面及双设备实际验收仍待完成。
...
src/local/sync-engine.ts 已实现账号绑定的持久化待发送队列、响应丢失重试、三方合并、冲突暂停与明确选择、本地并发编辑保留、退出后忽略在途响应。通过注入传输层的单元测试验证。尚未接入页面或真实 Supabase 传输，不能将此视为自动同步已可用。

## 页面集成进展

同步控制已连接原版导出/恢复与 Redux 订阅，支持首次选择、本地编辑防抖上传、15 秒轮询、恢复联网重试、退出停止、冲突整份选择及备份。浏览器模拟服务测试已走通原版编辑器添加后上传、远端修改拉取并显示、退出流程。仍不等同真实 Supabase 或两台设备验收。
