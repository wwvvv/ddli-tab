...
生产依赖检查命令：npm run test:production。它创建独立临时目录，只复制构建所需源码，不复制 .env、node_modules 或旧构建产物；执行 npm ci --omit=dev 后构建并验证关键输出，最后删除本次临时目录。已在 Windows + Node 22.11.0 下通过，只有 15 个安装包。Linux 容器验证尝试因 Docker Hub TLS 握手超时中断，尚不能宣称 Linux 云环境验证通过；GitHub CI 已配置相同检查。
